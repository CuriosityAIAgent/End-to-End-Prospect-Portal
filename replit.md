...
- `POST /file-notes/extract-profile` (operationId `extractFileNoteProfile`) is a sibling stateless AI endpoint that reads the note (+ engaged coverage) and returns `{values:[{key,value}]}` for ONLY the client-profile fields the note clearly addresses (omits anything not covered; never fabricates). The route validates with safeParse→400, parses the model JSON defensively, **filters returned keys to the request's `fields` allow-list and drops empty values** (a mass-assignment guard so a prompt-injected model can't write arbitrary keys into the assessment `data` blob), and returns 200 with `{values:[]}` for a legitimately-empty extraction (502 only on model/parse failure).
...
- Populate client profile from note (assessment workspace only): below the file note, a "Populate profile from note" action runs the `extract-profile` AI endpoint over the note (+ engaged coverage) and presents the suggested values per client-profile (Section 1) field with checkboxes — empty fields default-ticked, fields that would overwrite an existing value left unticked with an "Overwrites existing" badge, so applying is opt-in. Accepting merges the chosen values into the assessment `data` blob via a single batched autosave. Surfaced through the shared panel's optional `profileExtraction` prop, so the prospect file (which has no client-profile form) never shows it.
