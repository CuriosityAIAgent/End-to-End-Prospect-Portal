...
export function prepResponseSpec(): string {
  return [
    ...
    "  },",
    "APPROACH — the ONLY goal of every email and call is to secure a short (~30 minute) introductory meeting. Get to that ask fast. No product pitch, no feature lists, no paragraphs of what the bank offers — that wordiness kills it. Voice: a senior JPMorgan Private Bank advisor who looks after financial executives and sponsors (hedge funds / private equity).",
    '  "approach": {',
    ...
    '    "call": [ exactly 3 variants { "id": "call-1", "label": string, "rationale": string, "newsHook": string|null, "opener": string, "flow": string[] (3-5 beats) } ] — same three angles; the `opener` names who you are (JPM Private Bank, looking after financial sponsors) in one breath and the `flow` drives straight to asking for the 30-minute meeting. Keep every beat one short line;',
